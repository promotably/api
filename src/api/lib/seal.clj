...
(def ^{:doc "Algorithm to seed random numbers."
       :private true}
  seed-algorithm
  "SHA1PRNG")

(def ^{:doc "Algorithm to generate a HMAC."
       :private true}
  hmac-algorithm
  "HmacSHA256")
...
(def ^{:doc "Type of encryption to use."
       :private true}
  crypt-type
  "AES")

(def ^{:doc "Full algorithm to encrypt data with."
       :private true}
  crypt-algorithm
...
(defn hmac-sha1
  "Generates a Base64 HMAC with the supplied key on a string of data."
  [key data]
  (let [mac (Mac/getInstance hmac-sha1-algorithm)]
    (.init mac (SecretKeySpec. key hmac-sha1-algorithm))
    (base64-encode (.doFinal mac data))))
